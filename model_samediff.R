...
if (Sys.info()['nodename'] == 'daniel-desktop'){
	...
	model_dir = '~/apc_store/pitchbias/model/samediff'
} else if (Sys.info()["nodename"] == 'Ethans-MacBook-Air.local') {
  data_dir = '/Users/Ethan/Documents/GitHub/pitchbias/data'
  model_dir = '/Users/Ethan/Documents/GitHub/pitchbias/model/samediff'
...
load(file.path(data_dir, "clean_same_diff_data.RData"))
...
png(file.path(model_dir, "residuals_vs_fitted_values.png"))
plot(model, 
type=c("p", "smooth"), 
...
# Scale location plot
png(file.path(model_dir, "scale_location.png"))
...
png(file.path(model_dir, "binned_residuals.png"))
arm::binnedplot(fitted(model), residuals(model))
dev.off()

# Create analysis plots
png(file.path(model_dir, "all_effects.png"))
plot(allEffects(model))
dev.off()

png(file.path(model_dir, "trial_type.png"))
plot(predictorEffect("trial_type", model))
dev.off()

png(file.path(model_dir, "center_freq.png"))
plot(predictorEffect("center_freq", model))
dev.off()

png(file.path(model_dir, "freq_diff.png"))
