fit_sensory_model <- function(data_updown, data_samediff, version=1, starting_vals=c(1000, 0.05, 0, 0.75, 0)) {
...
model_fun <- function(p, data, version, switch) {
	...
	# First, calculate delta_f/f
	f_hat_1 = data_updown$tone_freq_int1+p[2]*(p[1]-data_updown$tone_freq_int1)
	internal_difference = (data_updown$tone_freq_int2-f_hat_1)/f_hat_1
	...
	f_hat_1 = data_samediff$tone_freq_int1+p[2]*(p[1]-data_samediff$tone_freq_int1)
	internal_difference = (data_samediff$tone_freq_int2-f_hat_1)/f_hat_1
	# Normalize that difference by the sensitivity to f2-f1
	internal_difference = internal_difference/(10^p[3])/sqrt(2)
	# Next, calculate k (TODO: from WHAT?! explain how k was calculated)
	k = numeric(length(internal_difference))
	k[data_samediff$freq_diff == 0.5] = 1.09
	k[data_samediff$freq_diff == 1.0] = 1.23
	k[data_samediff$freq_diff == 2.0] = 1.54
	k[data_samediff$freq_diff == 3.0] = 1.81
